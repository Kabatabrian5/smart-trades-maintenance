...
Follow-up overlap correction: the first spacing adjustment was on the outer workspace, but the inner scrollable trading `main` still allowed the Matches/Differs buttons to finish behind the fixed navigation. The mobile trading scroll container now reserves `pb-28` clearance, while the fixed navigation uses a stable 64px height. This directly addresses the overlap shown in the user's mobile screenshot. `npm run build` passes; redeploy and verify the action buttons are fully visible above the bottom navigation.

Final overlap diagnosis: the Matches/Differs action row is rendered inside the sibling trade `aside`, not the market `main`. The aside now also reserves `pb-28` on mobile, ensuring its action buttons remain above the fixed navigation. `npm run build` passes; redeploy the latest commit and verify Matches and Differs on a phone.
...
Deployment progress: the global `vercel --prod` command returned `Not authorized` because that PowerShell session could not use the correct CLI authorization. The authenticated CLI identity `kabatabrian5-7324` has access to the `smart-trades-maintenance` project. Deploying with `npx --yes vercel@59.0.0 --prod --yes --name smart-trades-maintenance` succeeded and aliased the production deployment to `https://smart-trades.site`.

Deposit implementation progress: added `api/deripay-deposit.ts`, which validates Kenyan phone numbers and USD amount limits, creates the documented Deripay HMAC-SHA256 signature, and calls `POST /api/v1/deposit` using the server-only `DERIPAY_API_KEY`. The Cashier Deposit form now has controlled phone and amount fields, working amount presets, a submitting state, and provider status feedback. Withdraw remains disabled because Deripay requires a separate initiate/verification-code/confirm flow. `npm run build` passes. Before live use, deploy and verify that Deripay accepts the current modern OIDC token plus Options account ID as its `userToken` and `loginid`; do not treat a successful initiation response as final settlement until transaction status is polled or confirmed by webhook.

Account header usability progress: the compact mobile account control now explicitly displays `Real` or `Demo` together with the active balance and currency. The full Deriv account ID remains visible in the expanded switcher rows, where both account type and balance are shown. Account balances are initialized from the selected account response and updated from balance messages. `npm run build` passes after this change.

Deripay app ID correction: Deripay reported that the API key had no Deriv app ID. The deposit proxy was incorrectly falling back to the OAuth client ID `34bIcDF1RsEKSAbKFKimH`, but Deripay requires the separate numeric Deriv `app_id`. `api/deripay-deposit.ts` now requires a numeric server-side `DERIV_APP_ID` and will return a clear configuration error instead of sending the wrong ID. Add the numeric app ID from the Deriv dashboard to Vercel for Preview, Development, and Production. Keep `DERIV_CLIENT_ID` for OAuth; do not replace it with the numeric value.

Deripay provider confirmed: `https://www.deripay.site/` is the official provider site shared for this project. Its public Quickstart documents API keys in the `dpk_live_<id>:<secret>` format, `X-API-Key`, `X-Timestamp`, and HMAC-SHA256 `X-Signature` headers. The signed payload is `${timestamp}\n${method}\n${path}\n${sha256(body)}`. Deposit uses `POST /api/v1/deposit` with `phoneNumber`, `usdAmount`, `currency`, `loginid`, `userToken`, and `appId`; completion is polled through `GET /api/v1/transactions/{transactionId}` or received through a `transaction.completed` webhook. Withdrawals use `POST /api/v1/withdrawals/initiate`, then `POST /api/v1/withdrawals/confirm` with the Deriv verification code, followed by the same status polling.

Deripay onboarding requires a registered Deriv app with `read`, `trading_information`, and `payments` scopes, plus a Deripay API key and app/domain binding. The current Smart Trades OAuth flow uses modern OIDC/Options account data and stores an OIDC access token, while Deripay's Quickstart examples expect a Deriv `loginid` and user token compatible with its payment-agent flow. Before enabling payment calls, verify with Deripay whether the current OIDC token and modern account ID are accepted, or whether a separate scoped Deriv user token/legacy login mapping is required. Never send the current token to Deripay until that compatibility is confirmed.
