  ...
  return (
    <aside className="w-80 bg-[#121217] border-r border-[#22222c] flex flex-col h-full text-white font-sans select-none">
      ...
      <div className="flex-1 overflow-y-auto p-3 text-gray-500">
        {activeSubTab === 'summary' && (positions.length === 0 ? <div className="flex h-full flex-col items-center justify-center text-center"><div className="mb-3 flex h-12 w-12 items-center justify-center rounded-xl border border-[#262633] bg-[#1a1a22] text-gray-400 shadow-inner">📊</div><p className="text-xs font-medium text-gray-400">No positions yet</p><p className="mt-1 text-[10px] text-gray-600">Completed or open trades will appear here.</p></div> : <div className="space-y-2">{positions.map((position) => { const profit = position.profit ?? 0; return <div key={position.id} className="rounded-xl border border-[#262633] bg-[#1a1a22] p-3 text-xs"><div className="flex justify-between gap-2"><span className="font-bold text-white">{position.symbol}</span><span className={position.status === 'Settled' ? (position.result === 'won' ? 'text-emerald-400' : 'text-rose-400') : 'text-amber-300'}>{position.status}</span></div><div className="mt-1 text-gray-500">{position.contract} · #{position.id}</div><div className="mt-2 grid grid-cols-2 gap-2 text-[11px]"><span>Tick {position.ticksElapsed ?? 0}/{position.duration}</span><span className={profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}>P/L {profit.toFixed(2)} USD</span><span>Value {(position.contractValue ?? position.stake).toFixed(2)} USD</span><span>Payout {(position.payout ?? 0).toFixed(2)} USD</span></div></div>; })}</div>)}
        {activeSubTab === 'transactions' && (positions.length === 0 ? <div className="py-12 text-center text-xs text-gray-500">No transactions yet.</div> : <div className="space-y-2">{positions.map((position) => <div key={position.id} className="rounded-xl border border-[#262633] bg-[#1a1a22] p-3 text-xs"><div className="flex justify-between"><span className="font-bold text-white">#{position.id}</span><span className="text-gray-400">{position.status}</span></div><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-gray-400"><span>Stake <b className="block text-white">{position.stake.toFixed(2)} USD</b></span><span>Value <b className="block text-white">{(position.contractValue ?? position.stake).toFixed(2)} USD</b></span><span>Payout <b className="block text-white">{(position.payout ?? 0).toFixed(2)} USD</b></span><span>P/L <b className="block text-white">{(position.profit ?? 0).toFixed(2)} USD</b></span></div></div>)}</div>)}
        {activeSubTab === 'journal' && (positions.length === 0 ? <div className="py-12 text-center text-xs text-gray-500">No journal events yet.</div> : <div className="space-y-2">{positions.flatMap((position) => [{ id: `${position.id}-opened`, label: 'Position opened', detail: `${position.symbol} · Contract #${position.id}` }, ...(position.status === 'Settled' ? [{ id: `${position.id}-settled`, label: `Position settled · ${position.result === 'won' ? 'Won' : 'Lost'}`, detail: `Profit/loss ${(position.profit ?? 0).toFixed(2)} USD` }] : [])]).map((event) => <div key={event.id} className="rounded-xl border border-[#262633] bg-[#1a1a22] p-3 text-xs"><p className="font-bold text-gray-200">{event.label}</p><p className="mt-1 text-gray-500">{event.detail}</p></div>)}</div>)}
      </div>
